...
export async function upsertVisitorSession(
  ...
  const visitorValues = {
      ...
      approximate_latitude: payload.geo.latitude == null
        ? null : Math.round(payload.geo.latitude * 10) / 10,
      approximate_longitude: payload.geo.longitude == null
        ? null : Math.round(payload.geo.longitude * 10) / 10,
  ...
  const visitorQuery = existingVisitor
    ? client.from("vgi_visitors").update(visitorValues).eq("id", existingVisitor.id)
  ...
  const { data: visitor, error: visitorError } = await visitorQuery
    .select("id").single();
  if (visitorError) throw new Error(`Visitor upsert failed: ${visitorError.message}`);

  const { data: existingSession, error: sessionReadError } = await client
    .from("vgi_sessions").select("id,maximum_scroll_percent,converted")
  ...
  const sessionValues = {
      ...
      converted: existingSession?.converted === true || payload.converted === true,
      metadata: {},
      updated_at: now,
  };
  const sessionQuery = existingSession
    ? client.from("vgi_sessions").update(sessionValues).eq("id", existingSession.id)
  ...
  const { data: session, error: sessionError } = await sessionQuery
    .select("id").single();
  if (sessionError) throw new Error(`Session upsert failed: ${sessionError.message}`);
  return { visitorId: visitor.id, sessionId: session.id };
}

export async function refreshSessionCounts(client: Client, context: IngestContext) {
  ...
  if (pages.error || events.error) throw new Error("Session count refresh failed");
